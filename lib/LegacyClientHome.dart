...
class LegacyClientHome extends StatefulWidget {
  ...
  LegacyClientHome({Key? key, this.fromAdminDashboard = false}) : super(key: key);

  @override
  LegacyClientHomeState createState() => LegacyClientHomeState();
}

class LegacyClientHomeState extends State<LegacyClientHome> {
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    if (widget.fromAdminDashboard) const SizedBox(width: 12),
                    Container(
...
class LegacyClientDashboardScreen extends StatefulWidget {
  const LegacyClientDashboardScreen({Key? key}) : super(key: key);

  @override
  LegacyClientDashboardScreenState createState() {
    return LegacyClientDashboardScreenState();
  }
}

class LegacyClientDashboardScreenState extends State<LegacyClientDashboardScreen> {
  ...
  Future<void> _navigateToWidget(Widget page) async {
    ...
    // Keep project context warm, but don't block navigation on a full reload.
    unawaited(DataProvider().loadProjectDataForProject(projectId));
    if (!mounted) return;
...
class _LegacyLogoutButton extends StatelessWidget {
